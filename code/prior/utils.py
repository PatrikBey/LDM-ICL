...
def visualize_inference3D(gt, rec, template, filename='/data/gt_overlay.png', ):
    ...
    plt.imshow(numpy.rot90(numpy.where(gt[idx[0],:,:] > 0, 1, numpy.nan),1), cmap='binary_r', alpha = .75)
    ...
    plt.imshow(numpy.rot90(numpy.where(gt[:,idx[1],:] > 0, 1, numpy.nan),1), cmap='binary_r', alpha = .75)
